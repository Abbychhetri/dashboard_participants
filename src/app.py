...
from dash import Dash, dcc, html, Input, Output, dcc,html
...
#Seperating the data as it is all in single rows and renaming them
...
#Checking the dimension and data types of the different variables (columns)-initial data
print(data_2.dtypes)


num_of_rows = data_2.shape[0]
num_of_columns = data_2.shape[1]
print('')
print('The number of rows in the inital dataset are: ' + str(num_of_rows) + ' and the number of columns are: ' + str(num_of_columns))

#Finding out the total number of empty cells (null values) in each columns

data_2.isnull().sum()

#Since we have 601 empty cells in 3 columns, we will be dropping them using the drop.na

data_3=data_2.dropna().reset_index(drop=True)
data_3.isnull().sum(axis = 0)

#Changing the data type for the different variables
#Creating new columns - Date, Time and Day of the week (mon,tues, etc), time of the day (morning, afternoon...) from the Time Stamp columns


data_3 = data_3.astype({"Device_record_ID":'category',"Device_ID":'category',"Control_name":'category' ,"Description":'category'})
...
#Finally changing the data types of the 3 newly created columns (variables) and checking the data types

data_3 = data_3.astype({"day_of_week":'category',"time_of_day":'category',"Phases":'category',"Activity":'category'})
...
app.layout = html.Div([
    ...
    #                 dbc.CardBody([html.Div([
    #         ]),
    #                     dcc.Graph(id='table_data', figure={}),
